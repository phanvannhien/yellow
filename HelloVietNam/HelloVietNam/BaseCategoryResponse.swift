...
class BaseCategoryResponse: Serializable {
    ...
    override func fromJsonAnyObject(jsonAnyObject: AnyObject) {
        ...
        // children
        if let dataObjectChildren: Array = json["children"].arrayObject
        {
            var listSub: [CategoryChildren] = []
            for item in dataObjectChildren
            {
                let temp:CategoryChildren = CategoryChildren.newInstance() as! CategoryChildren
